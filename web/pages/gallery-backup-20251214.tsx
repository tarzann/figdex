...
export default function Home() {
    ...
    const loadIndexData = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          // Load all frames from all indices by default (use original data.data to get all actual files)
          console.log('Loading all frames from', data.data.length, 'indices...');
          const allFrames = [];
          const corruptedIndices: string[] = [];
          for (const file of data.data) {
            try {
              console.log(`Loading frames from index ${file.id} (${file.file_name})`);
              ...
              const fileFrames = Array.isArray(indexDataContent) 
                ...
                : [];
              console.log(`✅ Added ${fileFrames.length} frames from ${file.file_name}`);
  ...
  const autoLogin = () => {
    if (typeof window !== 'undefined') {
      const testUser = {
        email: 'ran.3dcube@gmail.com',
        api_key: 'indexo_00e302970218510348df296770ce00b8'
      };
      localStorage.setItem('figma_web_user', JSON.stringify(testUser));
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const user = localStorage.getItem('figma_web_user');
    if (!user) {
      console.log('No user found, applying auto login...');
      // Auto login for testing
      autoLogin();
      // Force immediate reload after setting user
      setTimeout(() => {
  ...
  const allGalleryThumbs = useMemo(() => {
    let idx = 0;
    console.log('Processing frames for gallery:', frames);
    ...
    return frames.map((frame) => {
      ...
      // This ensures each frame has a unique identifier even if names are duplicated
  ...
  // Clean up invalid favorites when frames change (after frames are loaded)
  useEffect(() => {
    if (allGalleryThumbs.length === 0) return; // Wait for frames to load
    
    const validThumbNames = new Set(allGalleryThumbs.map(t => t.thumb.thumbName));
    
    // Use functional update to avoid stale closure issues
    setFavorites((prev) => {
      const invalidFavorites = prev.filter(fav => !validThumbNames.has(fav));
      if (invalidFavorites.length > 0) {
        console.log(`🧹 Cleaned up ${invalidFavorites.length} invalid favorite(s) from old thumbName format`);
        return prev.filter(fav => validThumbNames.has(fav));
      }
      return prev; // No changes needed
    });
  }, [allGalleryThumbs]); // Only run when frames change
        ...
        <Box sx={{ p: 2, height: '100%', overflowY: 'auto' }}>
          ...
          <Box sx={{ mt: 4, p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }}>
            <Typography variant="caption" color="text.secondary">
              Showing {visibleThumbs.length} of {allGalleryThumbs.length} frames
            </Typography>
          </Box>
        </Box>
      </Box>
        ...
        <Box sx={{ px: 4, pb: 1 }}>
          <Box sx={{ mb: 1 }}>
            <Box display="flex" alignItems="flex-end" gap={2} sx={{ width: '100%', flexWrap: 'nowrap', overflowX: 'auto' }}>
              <Box sx={{ flex: '1 1 260px', minWidth: 180, maxWidth: 360 }}>
                <TextField
                  value={search}
              ...
              <Box sx={{ minWidth: 120, flexShrink: 0 }}>
                <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5, display: 'block', fontSize: '0.7em' }}>
                  Images per page
                </Typography>
                <TextField
                  select
              ...
              <Box sx={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5, display: 'block', fontSize: '0.7em', visibility: 'hidden' }}>
                  &nbsp;
                </Typography>
                <IconButton
                  ...
                  sx={{ 
                    height: 56,
                    width: 56,
                    border: showFavorites ? '1px solid' : '1px solid #e0e0e0',
                    borderColor: showFavorites ? 'primary.main' : 'divider',
                    bgcolor: showFavorites ? 'primary.main' : 'transparent',
                    color: showFavorites ? 'white' : 'action.active',
                    '&:hover': {
                      bgcolor: showFavorites ? 'primary.dark' : 'action.hover',
                    }
                  }}
          ...
          {/* Current View Info - part of header */}
          <Box sx={{ px: 0.5, pt: 0.5, mt: 1 }}>
            <Typography variant="body2" color="text.secondary" sx={{ wordBreak: 'keep-all' }}>
              {selectedIndex ? 
                ...
                `Showing ${visibleThumbs.length} total frames (${pagedThumbs.length} on page ${page}/${totalPages})`
              }
            </Typography>
          </Box>
        </Box>
